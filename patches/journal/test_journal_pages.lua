-- Page-by-page screenshots of a few Journal messages (the widest lines and the three kinds of group).
-- Otherwise the same as test_journal.lua: every scenario in test_journal_cases.lua (made by make_tests.py) writes its story flags, uses the
...
function TEST(f)
  ...
  elseif phase == "case" then
    ...
    while PAGES and not (function() for _, p in ipairs(PAGES) do if p == n then return true end end end)() do
      n = n + 1
      if n > #CASES then w("finished"); done(); phase = "end"; return end
    end
    local c = CASES[n]
  ...
  elseif phase == "close" then
    ...
    if (f - t0) % 70 == 0 then
      ...
      shot(string.format("page_%d_%02d", n, taps))
      tap(K.A); taps = taps + 1
