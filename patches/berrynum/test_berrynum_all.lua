...
at(0, function()
  ...
  local ids = {}
  for id = 704, 727 do ids[#ids + 1] = id end
  for id = 762, 765 do ids[#ids + 1] = id end
  for id = 133, 175 do ids[#ids + 1] = id end
  local k = 0
  for _, id in ipairs(ids) do
    if k < cap then emu:write16(base + k * 4, id); emu:write16(base + k * 4 + 2, 1 ~ key); k = k + 1 end
  end
...
function TEST(f)
  ...
  if d >= 440 and (d - 440) % 24 == 0 then
    local k = (d - 440) // 24
    if k < n - 1 then
      if k >= 40 then shot(string.format("all_%02d", k)) end
      tap(K.DOWN)
    elseif k == n + 2 then
      shot("all_end"); w("no crash over " .. n .. " berries; cb2 " .. string.format("%08X", cb2())); done()
    end
  end
end
