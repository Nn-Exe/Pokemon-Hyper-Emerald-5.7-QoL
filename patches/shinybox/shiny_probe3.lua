...
local log = io.open(dir .. "shiny3_log.txt", "w")
...
local function dump()
  log:write("== gBattleMons[0..1] @02024084 (0x58 each) ==" .. NL)
  for b = 0, 1 do
    local base = 0x02024084 + b * 0x58
    local s = string.format("mon %d:", b)
    for i = 0, 0x57, 4 do s = s .. string.format(" %02x:%08x", i, emu:read32(base + i)) end
    log:write(s .. NL)
  end
  log:write(string.format("party enemy[0] pid=%08x otid=%08x species=%04x level=%d",
    emu:read32(0x02024744), emu:read32(0x02024744 + 4), emu:read16(0x02024744 + 0x20), emu:read8(0x02024744 + 0x54)) .. NL)
  log:write("== sSpritePaletteTags 0x03000CF0 ==" .. NL)
  local t = ""
  for i = 0, 15 do t = t .. string.format(" %2d:%04x", i, emu:read16(0x03000CF0 + i * 2)) end
  log:write(t .. NL .. "== done ==" .. NL)
  log:flush()
end
...
callbacks:add("frame", function()
  ...
  if phase == "walk" and f > 2600 then
    ...
    if f > 12000 then
      local fh = io.open(dir .. "shiny3_done.txt", "w"); fh:write("no battle"); fh:close(); phase = "over"
    end
  elseif phase == "wait" then
    if menuReady() and f > lastAction + 150 then
      emu:clearKey(A)
      emu:screenshot(dir .. "shiny3_battle.png")
      dump()
      local fh = io.open(dir .. "shiny3_done.txt", "w"); fh:write("done"); fh:close()
      phase = "over"
    else
      if (f - battleStart) % 60 == 30 then emu:addKey(A) end
      if (f - battleStart) % 60 == 34 then emu:clearKey(A) end
      if f > lastAction + 4000 then
        local fh = io.open(dir .. "shiny3_done.txt", "w"); fh:write("timeout"); fh:close(); phase = "over"
      end
    end
  end
end)
