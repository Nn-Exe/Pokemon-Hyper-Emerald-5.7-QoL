...
local log = io.open(dir .. "shiny4_log.txt", "w")
...
local function dump()
  log:write("== EWRAM scan for the healthbox palette (0000 2925 ff7f 5a67) ==" .. NL)
  local a = 0x02000000
  while a < 0x02040000 do
    if emu:read16(a) == 0x0000 and emu:read16(a + 2) == 0x2529 and emu:read16(a + 4) == 0x7fff
       and emu:read16(a + 6) == 0x675a then
      log:write(string.format("hit %08x", a) .. NL)
    end
    a = a + 2
  end
  log:write("== done ==" .. NL)
...
callbacks:add("frame", function()
  ...
  if phase == "walk" and f > 2600 then
    ...
    if f > 12000 then
      local fh = io.open(dir .. "shiny4_done.txt", "w"); fh:write("no battle"); fh:close(); phase = "over"
    end
  elseif phase == "wait" then
    if menuReady() and f > lastAction + 150 then
      emu:clearKey(A)
      emu:screenshot(dir .. "shiny4_battle.png")
      dump()
      local fh = io.open(dir .. "shiny4_done.txt", "w"); fh:write("done"); fh:close()
      phase = "over"
    else
      if (f - battleStart) % 60 == 30 then emu:addKey(A) end
      if (f - battleStart) % 60 == 34 then emu:clearKey(A) end
      if f > lastAction + 4000 then
        local fh = io.open(dir .. "shiny4_done.txt", "w"); fh:write("timeout"); fh:close(); phase = "over"
      end
    end
  end
end)
