...
local log = io.open(dir .. "fly2_log.txt", "w")
...
at(t + d, function() emu:screenshot(dir .. "fly2_pick.png") end)
tap(A, t + d + 10)                               -- fly!
for i = 1, 8 do
  at(t + d + 10 + i * 60, function() w(string.format("+%ds: %s", i, where())) end)
end
at(t + d + 300, function() emu:screenshot(dir .. "fly2_after.png") end)
tap(B, t + d + 320)                              -- B must still close the map
at(t + d + 400, function() w("after B: " .. where()) end)
at(t + d + 420, function() emu:screenshot(dir .. "fly2_closed.png") end)
at(t + d + 440, function() local fh = io.open(dir .. "fly2_done.txt", "w"); fh:write("done"); fh:close() end)
