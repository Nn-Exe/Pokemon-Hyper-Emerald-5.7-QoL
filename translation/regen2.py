"""Regenerate corpus with UNALIGNED pointer scan; produce delta worklist."""
...
def parse(t, maxlen=3000):
    ...
    while i < n and i-t < maxlen:
        b = data[i]
        if b == 0xFF: return out, i-t, nh, ns
        if b in LEADS:
            ...
            i += 2; continue
        if b == 0xFD: out.append(f'[BUF{data[i+1]:02X}]'); i += 2; continue
        if b == 0xFC:
            sub = data[i+1]; na = EXT_ARGS.get(sub, 0)
            out.append(f'[FC{bytes(data[i+1:i+2+na]).hex().upper()}]'); i += 2+na; continue
        if b in (0xF8, 0xF9): out.append(f'[{b:02X}{data[i+1]:02X}]'); i += 2; continue
...
# UNALIGNED pointer scan: find all bytes 0x08/0x09 then check
targets = set()
for hb in (b'\x08', b'\x09'):
    i = data.find(hb, 3)
    while i != -1:
        t = int.from_bytes(data[i-3:i], 'little') | ((hb[0]-8) << 24)
        # value = data[i-3..i+1] little endian with high byte hb -> offset
        if 0 < t < n:
            targets.add(t)
        i = data.find(hb, i+1)
print("unaligned targets:", len(targets))
...
prev_wl = json.load(open(sp + r"\worklist2.json", encoding='utf-8'))
prev_addrs = set()
for _, addrs in prev_wl.items(): prev_addrs.update(addrs)

work = {}
for t in sorted(targets):
    if t in prev_addrs: continue
    r = parse(t)
...
for a, t in work.items(): by_text[t].append(a)
print("delta strings:", len(work), "unique:", len(by_text))
print("delta hanzi:", sum(sum(1 for c in t if '一'<=c<='鿿') for t in by_text))
json.dump({t: addrs for t, addrs in sorted(by_text.items(), key=lambda kv: min(kv[1]))},
          open(sp + r"\worklist3.json", 'w', encoding='utf-8'), ensure_ascii=False, indent=0)
# quick samples
for t in list(by_text)[:10]:
    print(' ', t[:70])
