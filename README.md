...
| **Auto Run** — run without holding B (hold B to walk). Saved with your options, respects Running Shoes and map rules. It used to sit on R; R now opens the DexNav. | START → Option → **Auto Run** (replaces Button Mode) | ![](docs/showcase/option-autorun.png) |
...
| **Nature changer, no quiz** — the Unova Gym Leader in the Rustboro Trainer's School hands out Galar Mints that change a Pokémon's nature, normally only after a run of random true/false questions. Talking to him now opens the Mint offer straight away, and it stays repeatable: pick a nature, pick a Pokémon, done. The summary's **Nature** line shows the new nature too (the hack changed the stats but kept displaying the old one). | Talk to him in Rustboro | ![](docs/showcase/mint-nature.png) ![](docs/showcase/nature-summary.png) |
| **Rare Candy NPC** — a person in Petalburg City's Poké Mart offers you **999 Rare Candies, once**. He asks first; say yes and that is his whole stash. A candy *shop* was not possible: the hack soft-resets on any mart list it does not own. Contributed by [@anibalribeiro](https://github.com/anibalribeiro) (PR #1). | Talk to him in the Petalburg Mart | |
...
| **Gold healthbox for shinies** — any shiny Pokémon in battle gets a gold box: a wild one, a trainer's, and your own (in double battles too), so you can tell at a glance before the sprite finishes appearing. Only the box's fill changes; names, HP numbers and both bars stay as they are. Uses the game's own shiny check, so it matches the sprite exactly. | — | ![](docs/showcase/shiny-box.png) ![](docs/showcase/shiny-box-yours.png) ![](docs/showcase/shiny-box-zoom.png) |
| **DexNav screen** — lists every wild Pokémon of the map you are standing on: Land, Water, Rock Smash and Fishing, one row per species with its icon, level range and habitat, seven per page. Read-only — the same data as the guide site, without leaving the game. Appears once you have the Pokédex, in the Safari Zone too. | START menu → **DexNav**; ←/→ or L/R to page; B to close | ![](docs/showcase/dexnav-menu.png) ![](docs/showcase/dexnav-list.png) ![](docs/showcase/dexnav-city.png) |
...
| **DexNav search & chain** — on the DexNav screen (which also shows each species' Search Level) a red frame marks the selected species and **A** registers it, dropping you straight back to the field. A framed bar at the bottom of the field shows what you are hunting: its icon, a 0–3 star rating (how many perfect IVs it will have), the level and ability it will have, **No.** — how many you have caught or beaten in a row — **SL**, that species' Search Level, and an arrow toward the shaking patch (sideways until you are lined up, then up or down). Works like Pokémon Unbound's DexNav: every encounter of the tracked species raises its Search Level (0–999, kept across restarts, never reset), which raises the odds of an egg move (up to 83%), a held item (up to 12%) and perfect IVs (the stars — rarer than Unbound, and a chain of 10 / 20 is needed for 2 / 3 stars; one time in 500 all six are perfect, shown as three gold stars), and adds shiny rolls (about 0.33% at SL 255 and 0.61% at SL 999; the Shiny Charm multiplies that by 5). **While hunting a Land or Water species, a patch near you shakes** (with the Pokémon's cry when it appears) — rustling grass, a ripple on water, dust on a cave floor or sand. Step on it to meet the hunted Pokémon (Repel doesn't stop it); every other tile gives the map's normal Pokémon. Fishing species surface as a water ripple too (surf onto it); Rock Smash hunts work on Rock Smash encounters instead. The chain adds +1 level per 5 in a row and extra shiny rolls on the 50th and 100th. As in Omega Ruby / Alpha Sapphire, running, losing, the Pokémon fleeing, walking away from the patch (which also ends the search — register again), leaving the area or getting into any other battle (a normal wild Pokémon included) ends the chain, and closing the game resets it. No hidden abilities: this hack has none. | **R** in the field (or START → **DexNav**), D-pad to choose, **A** to register — **A** on the registered one unregisters | ![](docs/showcase/dexnav-register.gif) ![](docs/showcase/dexnav-patch.gif) ![](docs/showcase/dexnav-bar.png) ![](docs/showcase/dexnav-gold.png) |
...
| **Journal** — a key item, handed to you automatically, that tells you what to do next in the story, from Prof. Birch to Volo: the Hoenn story, the post-game missions on Steven's Island, Sinnoh and the Lost Artifacts quest. It reads the progress flags the game already keeps, so it never gets out of step and saves nothing of its own. For tasks you can do in any order it adds a progress line and what's left: the Sinnoh Gym Leaders still to beat, the Tapu trials, and the next of the 17 Plates with a hint to where it lies. Every step was traced in the game's own scripts, and it copes with things done out of order (Giratina battled early, a step skipped). | **Journal** in Key Items, or register it to **SELECT** | ![](docs/showcase/journal.png) ![](docs/showcase/journal-plates.png) ![](docs/showcase/journal-bag.png) |
| **Quest Log** — using the Journal opens a chapter grid (each chapter with its done/total), then a checklist of the whole story, one chapter a page (Hoenn, Post-game, Sinnoh, Lost Artifacts): a tick on what you have done, a red arrow on the current objective, a diamond on side objectives you can do in any order, and **???** for what is still ahead, so nothing is spoiled. **A** on a row reads the full objective (with the gyms, trials or Plates still missing); the header counts what is done in that chapter. It uses the Journal's own rules, so the two never disagree. A fifth chapter, **Legends**, lists every legendary and mythical Pokémon in National Dex order (with a silhouette): ticked once caught, named once seen, with where to find it. Its first row, **Gotta catch 'em all!**, is ticked with a Master Ball once you have caught them all. The Legends include the Ultra Beasts. A sixth, **Key Items**, lists the 55 key items in story order, named from the start (dimmed until you have them), ticked once you have them, with where each comes from. A seventh, **Side Content**, ticks off the Hisuian trades, gift Pokémon, other regions' hidden starters and side stories, with a red star on the ones whose Pokémon is always shiny. | **Journal**, or **R** in the Start menu (the "R Journal" box at the top left); then **L/R** or **←/→** chapter, **↑/↓** row, **A** read, **B** close | ![](docs/showcase/questlog-menu.png) ![](docs/showcase/questlog-grid.png) ![](docs/showcase/questlog.png) ![](docs/showcase/questlog-legends.png) ![](docs/showcase/questlog-keyitems.png) ![](docs/showcase/questlog-side.png) ![](docs/showcase/questlog-detail.png) |
