...
| **Party editor** — an **Edit** action in the party menu opens a screen for the selected Pokémon: page 1 changes its nature (by name, with the game's own up/down stat arrows) and the six IVs, page 2 the six EVs (with a running `EV total n/510`). The nature shown is the Pokémon's *effective* one — its Mint override if it has one, otherwise its own — so the screen agrees with the summary. Changes are written straight into the party and the stats are recalculated on the spot; the summary's **Nature** line follows an edit (see `patches/naturefix/`). Note the summary shows *computed stats*, not IVs/EVs, so IV edits are only visible as the stats moving — and at low level that is small (a maxed IV at Lv 12 is about +2). No item and no save change. Like the relearner's *Moves*, it is hidden when the Pokémon knows four field moves (the action list holds only eight). | Party menu → **Edit**; **UD** row, **LR** ±1, **L/R** ±5, **START** page, **B** done | ![](docs/showcase/party-editor.png) ![](docs/showcase/party-editor-evs.png) |
| **Rare Candy NPC** — a person in Petalburg City's Poké Mart offers you **999 Rare Candies, once**. He asks first; say yes and that is his whole stash — he has nothing left after that. A candy *shop* was not possible: the hack soft-resets on any mart list it does not own (`BuyMenuTryMakePurchase` jumps to hack code at `0x096FFF00`). | Talk to him in the Petalburg Mart | |
...
python patches/candynpc/candynpc_patch.py     out25.gba out26.gba   # Rare Candy NPC in Petalburg's Poke Mart
python patches/partyedit/partyedit_patch.py   out26.gba out27.gba   # party-menu editor for nature, IVs and EVs
python patches/naturefix/naturefix_patch.py   out27.gba out28.gba   # GetNature honours the hack's nature override
python tools/romdiff.py create original.gba out28.gba release/hyper-emerald-en-qol.hpatch
```
