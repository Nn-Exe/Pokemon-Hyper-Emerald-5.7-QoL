...
- **Added** Rare Candy NPC in Petalburg City's Poké Mart: he offers **999 Rare Candies, once**. He asks yes/no first, and once you accept he is empty for good (flag `0x4F1F` records it, so answering "no" leaves him able to ask again). A candy *shop* was not possible: the hack soft-resets on any mart list it does not own (`BuyMenuTryMakePurchase` jumps to hack code at `0x096FFF00`). `patches/candynpc/`
