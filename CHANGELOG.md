...
## 2026-09-21
- **Changed** Release v1.5 published (everything below since v1.4): the party editor, the Rare Candy NPC, and the nature-display fix. Rebuilt `release/hyper-emerald-en-qol.hpatch` and `release/CHECKSUMS.txt`; the patch is generated against the original Chinese ROM, and applying it there reproduces the build that was tested in mGBA byte for byte (result sha1 `04b2443d…`).
- **Fixed** The summary's **Nature** line now follows a Mint or the party editor. The hack stores a nature override in `mon+0x1F` and `CalculateMonStats` uses it, but `GetNature` (0x0806D070) still computed `personality % 25`, so the stats moved while the summary kept showing the old nature. `GetNature` now returns a non-zero override and otherwise runs the original code, so untouched Pokémon display exactly as before. `patches/naturefix/`
- **Added** Party editor: an **Edit** action in the party menu opens a screen for the selected Pokémon — page 1 changes its nature (shown by **name**, with the game's own stat arrows, e.g. "Jolly ↑Spe ↓Sp.A") and the six IVs, page 2 the six EVs with a running `EV total n/510`. Writes go straight into the party and `CalculateMonStats` runs on every change; nothing is stored in the save. Exits to the field with no menu (the first cut used `CB2_ReturnToFieldWithOpenMenu` and dropped you into the START menu). Hidden when the Pokémon knows four field moves, exactly like the relearner's *Moves*. `patches/partyedit/`
- **Added** Rare Candy NPC in Petalburg City's Poké Mart: he offers **999 Rare Candies, once**. He asks yes/no first, and once you accept he is empty for good (flag `0x4013` records it, so answering "no" leaves him able to ask again). `giveitem` masks its amount to a byte, so the candies go over in four calls. A candy *shop* was not possible: the hack soft-resets on any mart list it does not own (`BuyMenuTryMakePurchase` jumps to hack code at `0x096FFF00`). `patches/candynpc/`
