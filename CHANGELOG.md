...
## 2026-09-23
- **Changed** Release v1.5 published (everything below since v1.4): the Journal and the Quest Log, the Rare Candy NPC, Hyper Training that shows, the nature display fix, the Sinnoh League attendant in English, berry numbers, Enamorus's name, gold boxes for your own shinies. Rebuilt `release/hyper-emerald-en-qol.hpatch` and `release/CHECKSUMS.txt`: generated against the original Chinese ROM, and applying it there gives the tested build byte for byte (result sha1 `84f24f1e…`). The build also reproduces from the v1.4 release through the patch chain in HANDOFF.
- **Fixed** Hyper Training (Champion Island): the EV-IV Display and the IV judges now show **31** for a trained stat, and the trained stats go up the moment he finishes (before, they only changed after taking the Pokémon out of the PC). The real IVs stay as they were, as in the official games, so breeding and Hidden Power are unchanged - the EV-IV screen keeps showing the real Hidden Power type. `patches/hypertrain/`
...
- **Added** Side Content: a red star after the rows whose Pokémon is always shiny (the hidden Snivy, Litten, Grookey and Scorbunny battles, the Drifloon gift, the cursed statue's Gengar), traced from the hack's own shiny routine in each script and checked in the emulator. `patches/questlog/`
- **Added** Side Content: the hidden powers, traced from the scripts - the Burning Soul (a one-try hidden cave at the foot of Mt. Chimney on Route 112: the Crimson Apostle's three-round trial; the cave collapses afterwards whether you finish or leave) and the Hero Greninja chain (beat Koga on Champion Island for his mark, pass the one-Greninja trial in the hidden Koga's Village through the Route 102 forest, Ash's challenge on Route 101, the note that leads to Steven's residence). 44 rows. `patches/questlog/`
